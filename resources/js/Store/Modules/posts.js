...
export default {
    ...
    mutations: {
        ...
        updatePostCommentReply(state, reply) {
            ...
            const comment = post.comments.find((c) => c.id === reply.comment_id);
    ...
    actions: {
        async fetchPosts({ commit }, offset = 0) {
            await axios.get(route("posts.index", { offset })).then((response) => {
        ...
        async storeComment({ commit }, { postId, body }) {
            ...
            await axios.post(route("posts.comments.store", postId), { body }).then((response) => {
                console.log('the response is', response);
                
                if (response.data.success) {
                    console.log("[ACTION] stored comment: Comment=", response.data.data);
        ...
        async storeCommentReply({ commit }, { id, body }) {
            await axios.post(route("comments.reply", id), { body }).then((response) => {
                console.log('the response is', response);
        ...
        async updateComment({ commit }, { id, body }) {
            ...
            await axios.post(route("comments.update", id), { body, _method: "PUT" }).then((response) => {
                console.log('the response is', response);
                const comment = response.data.data;
                if (response.data.success) {
                    console.log("[ACTION] updated comment: Comment=", comment);
        ...
        async deletePostComment({ commit }, {id, postId, commentId}) {
            ...
            await axios.delete(route("comments.destroy", id)).then((response) => {
                if (response.data.success) {
                    if (commentId) {
                        commit("deletePostCommentReply", {id, postId, commentId});
                    } else {
