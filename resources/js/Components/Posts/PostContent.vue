<template>
  <div>
    <div v-show="isBodyNotEmpty" id="post-body" class="mb-3">
      <p class="" v-html="stripHashtag"></p>
    </div>
    <div
      v-show="post.attachments.length > 0"
    ...
    >
      <div v-for="(attachment, index) in post.attachments" :key="post.id">
        <template
          v-if="
            post.attachments.length <= 4 || (post.attachments.length > 4 && index < 3)
          "
        >
          <ImageAttachment
